...
def run_search(term):
    ...
    prompt = f"""أنت خبير علوم حاسب ومترجم تقني، تشرح المصطلحات التقنية باللغة العربية الفصحى الأكاديمية بدقة علمية وأسلوب طبيعي غير حرفي، لطالب علوم حاسب.

المصطلح: {term}

قواعد الدقة والمصطلحات:
- افهم المفهوم التقني الدقيق لهذا المصطلح تحديدًا قبل الشرح، ومَيّزه بوضوح عن أي مفهوم مشابه له (مثل Process/Thread، Authentication/Authorization، Compiler/Interpreter، Encryption/Hashing) بلا تعريف عام يصلح للاثنين معًا.
- استخدم المصطلح العربي التقني الصحيح المتعارف عليه أكاديميًا، لا ترجمة حرفية كلمة بكلمة (مثال مرفوض: ترجمة "Data Format" إلى "تعبير عن البيانات"؛ والصحيح "تنسيق لتمثيل البيانات"). طبّق نفس المبدأ على أي مصطلح آخر، والتزم بنفس الترجمة المختارة في كل أجزاء الإجابة.
- أضِف المصطلح الإنجليزي الأصلي بين قوسين بعد أول ذكر للترجمة العربية فقط عند احتمال لبس فيها.
- لا تخترع أي حقيقة تقنية أو رقمًا غير مؤكد؛ اذكر فقط ما هو معروف ومؤكد.
- عدّل أسلوب كل قسم بما يلائم طبيعة هذا المصطلح تحديدًا (لغة برمجة، مفهوم نظري، بروتوكول، خوارزمية، أداة، هجوم أمني...)؛ ممنوع تكرار نفس القالب اللغوي العام لكل المصطلحات.

قواعد اللغة (إلزامية):
- النص كله خارج الكود يجب أن يكون عربيًا فصيحًا 100%، عدا: اسم المصطلح نفسه، أسماء لغات/تقنيات معروفة (مثل Python أو CSS)، أو المصطلح الإنجليزي بين قوسين — وتُكتب ككلمة إنجليزية مستقلة بمسافة قبلها وبعدها، بلا دمجها مع حروف عربية (مثل "وسوم HTML" لا "HTML tags").
- ممنوع منعًا باتًا أي حرف من أي لغة ثالثة غير العربية والإنجليزية (روسية، صينية، يابانية، كورية، عبرية، إلخ)، ولو كلمة واحدة. راجع كل كلمة قبل التسليم.

قواعد الإيجاز: النقاط 1 و2 و4 بحد أقصى جملتين قصيرتين لكل نقطة (~30 كلمة)، وجملة الشرح بعد الكود في النقطة 3 بجملة واحدة قصيرة فقط.

التنسيق: ابدأ مباشرة بالعنوان "1. التعريف المبسط" بلا أي مقدمة، والتزم حرفيًا بالعناوين التالية بدون إضافة أو حذف:

1. التعريف المبسط: تعريف علمي دقيق ومختصر جدًا، بالمصطلح العربي الصحيح، مميّزًا عن أي مفهوم مشابه له.
2. مثال من الحياة الواقعية: تشبيه واحد يعكس الخاصية التقنية الفعلية للمصطلح، لا تشبيهًا عامًا فضفاضًا.
3. نظرة سريعة: إن كان للمصطلح صياغة برمجية فعلية (لغة، مكتبة، أمر، دالة...)، أعطِ كودًا قصيرًا (٢-٤ أسطر) داخل code block، صحيحًا نحويًا ويوضّح المصطلح تحديدًا، متبوعًا بجملة واحدة تشرح ماذا يفعل. إن لم يكن كذلك، أعطِ بدلاً من الكود مثالًا تطبيقيًا عمليًا موجزًا جدًا بلا كود.
4. أين يُستخدم هذا المصطلح فعليًا؟ تطبيق حقيقي واحد أو اثنان فقط، مرتبطان تحديدًا بهذا المصطلح لا بمجاله العام.

اجعل الشرح منظمًا ومختصرًا ودقيقًا، متسق المصطلحات، وخاليًا من الترجمة الحرفية غير الطبيعية."""
...
"""
        ...
        loading_placeholder.empty()
        err_text = str(e)
        print(f"[run_search] API error: {err_text}")  # للتتبع في السجل فقط، لا يظهر للمستخدم
        if "429" in err_text or "rate_limit" in err_text.lower():
            st.error("⚠️ الخدمة مشغولة حاليًا بسبب الضغط الكبير على الطلبات. جرّبي مرة أخرى بعد قليل.")
        elif "503" in err_text:
            st.error("⚠️ الخدمة غير متاحة مؤقتًا. جرّبي مرة أخرى بعد قليل.")
        else:
            st.error("⚠️ صار خطأ أثناء الاتصال بالخدمة. جرّبي مرة أخرى بعد قليل.")
